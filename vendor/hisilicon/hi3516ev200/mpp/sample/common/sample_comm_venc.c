...
HI_VOID SAMPLE_COMM_VI_ReadFrameYUV420(void* data, HI_U8* pY, 
                                       HI_U8* pU, HI_U8* pV, 
...
static int SAMPLE_COMM_GetTimestamp()
{
    struct timeval tv;
...
HI_S32 SAMPLE_COMM_VI_GetVFrameFromYUV420(void* data, HI_U32 u32Width, HI_U32 u32Height, HI_U32 u32Stride, VIDEO_FRAME_INFO_S* pstVFrameInfo)
{
	...
	for(i=0;i<10;i++){
	    VbBlk = HI_MPI_VB_GetBlock(VB_VENC_POOLID, u32Size, NULL);
    ...
    pstVFrameInfo->stVFrame.u64VirAddr[0] = (HI_U64)pVirAddr;
    ...
    pstVFrameInfo->stVFrame.u64PTS = SAMPLE_COMM_GetTimestamp();
    pstVFrameInfo->stVFrame.u32TimeRef = u32TimeIndex;
    u32TimeIndex += 2;
    ...
    SAMPLE_COMM_VI_ReadFrameYUV420(data, pstVFrameInfo->stVFrame.u64VirAddr[0],
                             pstVFrameInfo->stVFrame.u64VirAddr[1], pstVFrameInfo->stVFrame.u64VirAddr[2],
    ...
    SAMPLE_COMM_VI_PlanToSemi(pstVFrameInfo->stVFrame.u64VirAddr[0], pstVFrameInfo->stVFrame.u32Stride[0],
                              pstVFrameInfo->stVFrame.u64VirAddr[1], pstVFrameInfo->stVFrame.u32Stride[1],
                              pstVFrameInfo->stVFrame.u64VirAddr[2], pstVFrameInfo->stVFrame.u32Stride[1],
                              pstVFrameInfo->stVFrame.u32Width, pstVFrameInfo->stVFrame.u32Height);

    int s32Ret;
    int VencChn = 0;
    s32Ret = HI_MPI_VENC_SendFrame(VencChn, pstVFrameInfo, 1000);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("HI_MPI_VENC_SendFrame chn[%d] failed with %#x!\n", VencChn, s32Ret);
    }
    //printf("%s %d\n",__func__,pstVFrameInfo->stVFrame.u64PTS);
    //printf("u32TimeIndex:%d\n",pstVFrameInfo->stVFrame.u32TimeRef);
